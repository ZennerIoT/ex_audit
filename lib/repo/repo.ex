defmodule ExAudit.Repo do
  @moduledoc """
  Adds ExAudit version tracking to your Ecto.Repo actions.

  The following functions are extended to detect if the given struct or
  changeset is in the list of `:tracked_schemas` given in `:ex_audit` config:
  ...
  executed, i.e., the functions are marked as overridable and the overridden implementations
  ...
  All normal `Ecto.Repo` options will work the same, however, there are
  additional options specific to `:ex_audit`:

   * `:ex_audit_custom` - Keyword list of custom data that should be placed in
     new version entries. Entries in this list overwrite data with the same keys
     from the `ExAudit.track/1` call
  ...
  defmacro __using__(_opts) do
    quote location: :keep do
      ...
      @doc """
      Decides based on config `tracked_schema` weather the current schema is tracked or not.
   ...
   * `:render_structs` if true, renders the _resulting_ struct of the patch for
     every version in its history.

     This will shift the ids of the versions one down, so visualisations are
     correct and corresponding "Revert" buttons revert the struct back to the
     visualized state.

     Will append an additional version that contains the oldest ID and the
     oldest struct known. In most cases, the `original` will be `nil` which
     means if this version would be reverted, the struct would be deleted.
     `false` by default.
  """
   ...
   * `:preload` if your changeset depends on assocs being preloaded on the
     struct before updating it, you can define a list of assocs to be preloaded
     with this option
  """
